...
import { setMouseHover, scrollFloating, scrollParallaxImages, animInAppear } from '../Funcs/animates';
...
class About extends Component {
  ...
  render() {
    return <main className='subpage-content'>{this.state.loaded ? this._renderContent() : 'Loading...'}</main>;
  }
}
      ...
      <div className='l-wrapper'>
        <div className='text-wrap'>
          {skillCategories.map((category, key) => {
            return <SkillCategories category={category} skillData={dataSkills} key={'skill-category-' + key} />;
          })}
        </div>
      </div>
    </section>
  ...
  return (
    <div className={`category-${category} skill-item-container`}>
      <div className='skill-part _title'>
        <div
          className='skill-logo jsAppearBtT'
          style={{ backgroundImage: 'url(' + skillSprites + ')', backgroundPosition: bgPosition }}
        ></div>
        <h3 className='f-subhead jsAppearBtT' style={{ color: color }}>
          {title}
        </h3>
        <div className='skill-level-wrap jsAppearBtT'>
          <div
            className='skill-level-bar is-disabled'
            style={{ width: skillLevel * 10 + '%', backgroundColor: color }}
          ></div>
        </div>
      </div>
      ...
      <div className='l-wrapper'>
        <ul className='l-row gap90 clear-fix'>
          {instaImages.map((item, key) => {
            return key < 6 ? (
              <InstagramItems link={item.link} caption={item.caption} images={item.images} key={'instagram-' + key} />
            ) : (
              ''
            );
          })}
    ...
    <li className='l-col l-col-4-12 l-col-m-6-12 insta-image-li'>
      <a href={link} target='blank'>
        <div className='insta-image-wrap'>
          <div
            className='insta-image jsAppearSlideToR'
            style={{ backgroundImage: 'url(' + images.standard_resolution.url + ')', paddingTop: '100%' }}
          ></div>
