...
import { cbTimeout, topBtnHandler } from '../Funcs/functions';
...
class Home extends Component {
  ...
  hndl_mainMenuFollowingMouse = event => {
    const { jsBtnLeft, jsBtnCenter, jsBtnRight, jsMenuBgLeft, jsMenuBgRight, jsGotoToyProject } = this;
    let mouse = {
      x: event.clientX,
      y: event.clientY,
    };
  ...
  hndl_gotoSlider = () => {
    const { jsLabSliderIndex, hndl_mainmenuScroll, homeToySliderThumbnails: thumbnails } = this;
  ...
  hndl_gotoTop_home = () => {
    const {
      ...
      homeToySliderThumbnails: thumbnails,
    } = this;
    const { history } = this.props;
  ...
  render() {
    const { isMobile } = this.state;
    if (isMobile) {
      return <content className='home-wrapper'>{this.state.loaded ? this._renderMobile() : 'Loading'}</content>;
    } else {
      return <content className='home-wrapper'>{this.state.loaded ? this._renderContent() : 'Loading'}</content>;
    }
  }
}
    ...
    return (
      <nav className='menu-wrapper clear-fix' ref='jsMainmenu'>
        <div className='menu-btn left' ref='jsLeft'>
          <button className='f-hugetitle item left' ref='jsBtnLeft' onClick={hndl_mainLeft}>
            {mainMenu[1]}
          </button>
        </div>
        <div className='menu-btn center' ref='jsCenter'>
          <button className='f-hugetitle item center' ref='jsBtnCenter' onClick={hndl_mainCenter}>
            {mainMenu[0]}
          </button>
        </div>
        <div className='menu-btn right' ref='jsRight'>
          <button className='f-hugetitle item right' ref='jsBtnRight' onClick={hndl_mainRight}>
            {mainMenu[2]}
          </button>
        </div>
      </nav>
      ...
      <div className='vertical-line' ref='jsMenuVerticalLine'>
        <div className='links'>
          {outLinks.map((item, key) => (
            <a className='link-item c-gray-dark f-eng f-normal' href={item.url} target='blank' key={`outlinks-${key}`}>
              {item.title}
            </a>
          ))}
        </div>
        ...
        <div className='bottom' id='jsGotoToyProject' ref='jsGotoToyProject'>
          <button className='view-toy-btn f-subhead f-eng-title' id='jsCodeLabBtn' onClick={hndl_gotoSlider}>
            TOY PROJECT
          </button>
        </div>
      </div>
    ...
    <ul className='l-row'>
      <li className='l-col l-col-8-12'>
        {mainMenu.map((item, key) => {
          return (
            <button
              className='f-bigtitle item'
              id={`jsMobileMenu0${key + 1}`}
              onClick={_mobileGoToPage}
              data-goto={'/' + item}
              key={item}
            >
              {item}
