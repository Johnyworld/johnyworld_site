...
import { scrollFadeIn, setMouseHover, scrollParallaxImages, scrollFloating, animInAppear } from '../Funcs/animates';
...
class WorkDetail extends Component {
  ...
  _nowLoading() {
    ...
    const { anim_loadingScreenOut, anim_loadingScreenSetFull, anim_headerIn } = this.props;
  ...
  _renderContent() {
    ...
    if (id === 'bigpicture-ent') {
      workDetailContent = <BigpictureEnt />;
    }
    if (id === 'fanclub-coin') {
      workDetailContent = <FanclubCoin />;
    }
    if (id === 'soo-clinic') {
      workDetailContent = <Soohan />;
    }
    if (id === 'krx') {
      workDetailContent = <Krx />;
    }
    if (id === 'samsung-pssd') {
      workDetailContent = <Pssd />;
    }
    if (id === 'camping-poster') {
      workDetailContent = <Camping />;
    }
    if (id === 'the-focus') {
      workDetailContent = <TheFocus />;
    }
    if (id === 'daylog') {
      workDetailContent = <Daylog />;
    }
        ...
        <div className='text-wrap title-area' id='jsTitleArea'>
          ...
          <div className='project-info jsTitleChildren jsAppearBtT'>
            <ul className='keywords f-normal'>
              ...
              {url ? (
                <li>
                  <a href={url} target='blank' className='btn-viewsite'>
                    사이트 보기 >
                  </a>
                </li>
              ) : (
        ...
        <div className='text-wrap'>
          <ul className='l-row'>
            <li className='l-col l-col-6-12 l-col-m-12-12'>
              <h2 className='f-title jsAppearBtT'>
                프로젝트 <br className='dis-m' />
                개요
              </h2>
            </li>
            <li className='l-col l-col-6-12 l-col-m-12-12'>
