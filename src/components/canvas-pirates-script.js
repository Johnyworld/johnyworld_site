...
import DefineShip from './pirates/define-ship';
import DefinePirates from './pirates/define-pirates';
import DefineFrigates from './pirates/define-frigates';
import CannonBall from './pirates/cannon-ball';
import { textBoard } from './pirates/interface';
import { keyStates, removeKeyStates } from './pirates/key-states';
...
const canvasPirates = () => {
    ...
    const renderTitle = () => {
        ...
        ctxPrts.fillText("스페이스바를 누르면 게임 시작", canvasPrts.width / 2, canvasPrts.height / 2 + 110);
    }
    ...
    const checkCollision = () => {
        ...
        for (let i = cannonBalls.length - 1; i >= 0; i--) {
            ...
            if (collisionDetection(playerA, tempCannonBall)) {
                ...
                console.log('TakenDamage: ', damageSum);
            }
        }
    ...
    // 바람 컨트롤 함수
    const windy = () => {
        let random = Math.floor(Math.random() * 7 - 3) / 40;
        WIND.x += random;
    }
    ...
    const runningGame = () => {
        ...
        if (window.location.pathname !== "/lab/modern-pirates") {
            cancelAnimationFrame(canvasFrames);
        }
    }
